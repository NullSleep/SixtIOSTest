...
final class DrawerPresentationController: UIPresentationController {
  ...
  private let maxFrame = CGRect(x: 0, y: 0,
                                width: UIWindow.root.bounds.width,
                                height: UIWindow.root.bounds.height + UIWindow.key.safeAreaInsets.bottom)
  ...
  @objc private func userDidPan(panRecognizer: UIPanGestureRecognizer) {
    ...
    let isUp = frameOfPresentedViewInContainerView.origin.y - maxFrame.origin.y
    let notUp = -(frameOfPresentedViewInContainerView.origin.y - nextOriginY)
    let area = dragDirection == .up ? isUp : notUp
    ...
    if newOffset >= 0 && canDragInProposedDirection {
      ...
      case .changed, .began:
        ...
        if newOffset != area &&
          draggablePosition == .open || draggablePosition.nextPosition(for: dragDirection) == .open {
